...
public class PlanetControl : MonoBehaviour
{
    ...
    [SerializeField] private string planetType = "Inconnue";
    ...
    #endregion
    #region Public Info Accessors

    // Display name (falls back to GameObject.name if empty)
    //public string PlanetName => string.IsNullOrEmpty(planetName) ? gameObject.name : planetName;

    public double PlanetMass => planetMass;

    // Return diameter in meters (physical)
    public double PlanetDiameter => planetDiameter;

    // Semi-major axis in meters (kepler.a)
    public double SemiMajorAxis => kepler.a;

    public void SetPlanetName(string name) => planetName = name;
    public string GetPlanetName() => planetName;
    public void SetPlanetType(string type) => planetType = type;
    public string GetPlanetType() => planetType;

    // Orbital period in days computed from a and mu.
    // Use muCentral if available, otherwise fall back to PhysicsConstants.MuSun.
    public double OrbitalPeriodDays
    {
        get
        {
            double mu = (sunScript != null) ? sunScript.GetMu() : PhysicsConstants.MuSun;
            if (kepler.a <= 0.0 || mu <= 0.0) return double.NaN;
            double periodSeconds = 2.0 * Math.PI * Math.Sqrt(Math.Pow(kepler.a, 3) / mu);
            return periodSeconds / 86400.0; // convert to days
        }
    }
